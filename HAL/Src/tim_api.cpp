...
void tim_enable(TIM_TypeDef *instance) {
    if (instance == TIM2) {
        ...
        HAL_NVIC_SetPriority(TIM2_IRQn, RTOS_ISR_DEFAULT_PRIORITY, 0); // This function sets the priority of the interrupt associated with the TIM peripheral
        HAL_NVIC_EnableIRQ(TIM2_IRQn);         // This function enables the interrupt request for the TIM peripheral. Once enabled, the microcontroller will respond to interrupts generated by TIM2. This function allows the processor to respond to the interrupt request when it occurs.
    } else if (instance == TIM3) {
        __HAL_RCC_TIM3_CLK_ENABLE();
        HAL_NVIC_SetPriority(TIM3_IRQn, RTOS_ISR_DEFAULT_PRIORITY, 0);
        ...
        HAL_NVIC_SetPriority(TIM5_IRQn, RTOS_ISR_DEFAULT_PRIORITY, 0);
